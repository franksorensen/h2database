...
import org.h2v1.security.auth.AuthenticationException;
import org.h2v1.security.auth.AuthenticationInfo;
...
public class Engine implements SessionFactory {
        ...
        if (ci.getProperty("JMX", false)) {
            try {
                Utils.callStaticMethod(
                        "org.h2v1.jmx.DatabaseInfo.registerMBean", ci, database);
            } catch (Exception e) {
    ...
    void close(String name) {
        if (jmx) {
            try {
                Utils.callStaticMethod("org.h2v1.jmx.DatabaseInfo.unregisterMBean", name);
            } catch (Exception e) {
                throw DbException.get(ErrorCode.FEATURE_NOT_SUPPORTED_1, e, "JMX");
            }
        }
